...
#' @aliases glmer_b
#' 
#' @title Bayesian Mixed Effects Regression
#' 
#' @description
#' Maximum  a posteriori estimation for linear and generalized linear 
#' mixed-effects models in a Bayesian setting.  **This is a wrapper for 
...
#' or the \code{lmer_alt} function from the \link[afex]{`afex-package`} package.)
...
#' *strongly* recommend its use, especially when later applying methods such 
#' as \code{update} and \code{drop1} to the fitted model (*such methods are 
...
#' @param fixef.prior a BLME prior of family \code{normal}, \code{t}, or 
...
#' @param resid.prior a BLME prior of family \code{gamma}, \code{invamma}, 
...
#' @param cov.prior a BLME prior or list of priors with allowable 
...
#' @param control a list (of correct class, resulting from \link[lme4]{lmerControl}() 
...
#' documentation in the \link[lme4]{`lme4-package`} package for details.
...
#' included by default.
#' @param weights an optional vector of \code{‘prior weights’} to be used in 
...
#' @param mustart optional starting values on the scale of the conditional 
#' mean, as in \link[stats]{glm}; see there for details.
#' @param etastart optional starting values on the scale of the 
...
#' its environment, it may not return *exactly* the same values on subsequent 
...
#' 
#'  Chung Y, Rabe-Hesketh S, Dorie V, Gelman A, Liu J (2013). “A nondegenerate penalized likelihood estimator for variance parameters in multilevel models.” _Psychometrika_, *78*(4), 685-709. doi:10.1007/s11336-013-9328-2 <https://doi.org/10.1007/s11336-013-9328-2>.
#'  
#' 
...
lmer_b <- function(...,
  ...
  # If caller didn't provide a fixef.prior explicitly, construct one as an *unevaluated call*
  if (is.null(dots$fixef.prior)) {
    if (.use_t_prior) {
      # Build: t(df=..., mean=..., scale=..., common.scale=...)
      dots$fixef.prior <- substitute(
        t(df = DF, mean = MEAN, scale = SCALE, common.scale = CS),
        list(DF = fixef_df, MEAN = fixef_mean, SCALE = fixef_scale, CS = fixef_common_scale)
      )
    } else {
      # Example: use default normal prior (you can parameterize similarly)
      dots$fixef.prior <- quote(normal)
      # or: quote(normal(sd = c(10, 2.5), common.scale = TRUE))
    }
  }
  
  # t/horseshoe priors must use ML, not REML
  if (is.null(dots$REML) && .use_t_prior) {
    dots$REML <- FALSE
  }
  
  # Call blmer
  do.call(blme::blmer, dots)
}

# lmer_b = function(formula, 
#                   data = NULL,  
#                   fixef.prior, 
#                   resid.prior,
#                   cov.prior = wishart,
...
#   if(missing(data)){
#     X = 
#       lme4::lFormula(formula)$X
#   }else{
#     X = 
#       lme4::lFormula(formula,data)$X
#   }
#   if(ncol(X) > 1)
...
#   
#   
#   if(missing(fixef.prior)){
#     if(ncol(X) > 1){
#       fixef.prior = 
#         t(df = 1e3,
#           mean = c(mean(y),numeric(ncol(X)-1)),
#           scale = 2.5^2 * s_y^2 / c(1.0,s_j)
#         )
#     }else{
#       fixef.prior = 
#         t(df = 1e3,
